  ...
  return (
    <div className="relative w-full pt-6 pb-2 group/graph">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-40 overflow-visible">
        ...
        <line x1="0" y1={avgY} x2={width} y2={avgY} stroke="#fb923c" strokeWidth="1" strokeDasharray="4 4" opacity="0.4" />
        <text x={width} y={avgY - 6} textAnchor="end" className="fill-orange-400/40 text-[20px] font-mono uppercase tracking-widest">Average: {avg} WPM</text>
        ...
        {points.map((p, i) => (
          <g key={i} className="group/point">
            <circle cx={p.x} cy={p.y} r="5" className="fill-card stroke-orange-400 stroke-2 opacity-0 group-hover/graph:opacity-100 transition-opacity" />
            <text x={p.x} y={p.y - 12} textAnchor="middle" className="fill-orange-400 text-[10px] font-mono opacity-0 group-hover/graph:opacity-100 transition-opacity pointer-events-none">{data[i]} WPM</text>
          </g>
        ))}
      </svg>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
      ...
      <div className="relative w-full max-w-md bg-card border border-border rounded-[32px] p-8 shadow-2xl animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 overflow-hidden">
        <div className="absolute -right-6 -bottom-6 opacity-[0.03] text-orange-400"><IconAlert size={160} /></div>
        <div className="relative z-10 flex flex-col items-center text-center gap-6">
          <div className="w-16 h-16 rounded-2xl bg-orange-400/10 flex items-center justify-center text-orange-400 shadow-inner"><IconAlert size={32} /></div>
...
export function StatsTab() {
  ...
  const [sortField, setSortField] = useState<"date" | "wpm">("date");
  ...
  useEffect(() => { 
    setData(getStats(key)); 
    setCurrentPage(1); // Reset page when duration changes
  }, [key, rev]);

  // Derived Data: Sorted and Paginated Records
  const sortedAndPaginated = useMemo(() => {
    const sorted = [...data.records].sort((a, b) => {
      if (sortField === "date") {
        ...
        return sortOrder === "desc" ? b.wpm - a.wpm : a.wpm - b.wpm;
      }
    });

    const start = (currentPage - 1) * pageSize;
    return sorted.slice(start, start + pageSize);
  }, [data.records, currentPage, sortField, sortOrder]);

  const totalPages = Math.ceil(data.records.length / pageSize);
    ...
    <div className="p-8 md:p-12 max-w-7xl mx-auto flex flex-col gap-10 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 rounded-2xl bg-orange-500/10 flex items-center justify-center text-orange-400 shadow-inner"><IconChart size={24} /></div>
            <p className="text-xs font-mono tracking-[0.3em] uppercase text-orange-400 font-bold">Performance</p>
          </div>
          <h1 className="text-5xl font-black text-foreground tracking-tight leading-none">Your <span className="text-orange-400">Progress</span></h1>
          <p className="text-lg text-foreground/40 leading-relaxed max-w-2xl">Analyze your typing speed and accuracy trends. Select a duration to filter your history.</p>
        </div>
        ...
        <div className="bg-card/40 border border-border p-1.5 rounded-2xl flex items-center gap-1 self-start">
          {([["15","15s"],["30","30s"],["60","60s"],["0","Full"]] as const).map(([k, l]) => (
            <button key={k} onClick={() => setKey(k)} className={`px-5 py-2 rounded-xl text-sm font-bold transition-all ${key === k ? "bg-orange-400 text-white shadow-lg shadow-orange-400/20" : "text-foreground/40 hover:text-foreground hover:bg-foreground/5"}`}>{l}</button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <div key={i} className="group relative bg-card/40 border border-border rounded-[32px] p-8 overflow-hidden transition-all hover:-translate-y-1">
            ...
            <div className="flex items-baseline gap-2"><span className="text-4xl font-black text-foreground">{c.val}</span><span className="text-sm font-bold text-foreground/30">{c.unit}</span></div>
          </div>
        ))}
      </div>
      ...
      {trend.length > 1 && (
        <div className="relative bg-card/40 border border-border rounded-[40px] p-8 overflow-hidden group">
          ...
          <div className="flex flex-col gap-4">
            <div><h3 className="text-xl font-bold text-foreground">Speed Progression</h3><p className="text-sm text-foreground/40 font-mono uppercase tracking-widest mt-1">Timeline analysis based on last {data.total} sessions</p></div>
            <DetailedGraph data={trend} avg={data.avg} />
          </div>
        </div>
      )}

      {/* ── HISTORY LIST WITH SORTING & PAGINATION ── */}
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between px-2 flex-wrap gap-4">
          <div className="flex items-center gap-6">
            ...
            {/* Sorting Filter */}
            <div className="flex items-center gap-2 bg-foreground/5 p-1 rounded-xl border border-border/50">
                <span className="text-[10px] font-mono font-bold text-foreground/20 uppercase px-2">Sort By</span>
                <select
                  ...
                  "
                >
                  <option value="date-desc" className="bg-card text-foreground">Newest First</option>
                  <option value="date-asc" className="bg-card text-foreground">Oldest First</option>
                  <option value="wpm-desc" className="bg-card text-foreground">Highest WPM</option>
                  <option value="wpm-asc" className="bg-card text-foreground">Lowest WPM</option>
                </select>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <button onClick={() => setModal({ open: true, type: "key" })} className="group flex items-center gap-2 px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-widest text-foreground/30 hover:text-red-400 hover:bg-red-400/10 transition-all border border-transparent"><IconTrash size={14} className="group-hover:scale-110" /> Clear This</button>
            <button onClick={() => setModal({ open: true, type: "all" })} className="group flex items-center gap-2 px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-widest text-foreground/30 hover:text-red-400 hover:bg-red-400/10 transition-all border border-transparent"><IconTrash size={14} className="group-hover:scale-110" /> Clear All</button>
          </div>
        </div>
        ...
        {data.records.length === 0 ? (
          ...
          <>
              ...
              {sortedAndPaginated.map((r, i) => {
                ...
                return (
                  <div key={i} className="group grid grid-cols-2 md:grid-cols-6 items-center bg-card/40 border border-border hover:border-orange-400/30 rounded-2xl px-6 md:px-8 py-5 transition-all hover:bg-card">
                    <div className="text-xs font-mono font-bold text-foreground/20">#{String(data.records.length - ((currentPage - 1) * pageSize + i)).padStart(2, '0')}</div>
                    ...
                    <div className="flex flex-col md:block"><span className="text-lg font-black text-orange-400">{r.wpm}</span><span className="text-[10px] font-bold text-foreground/30 ml-1 uppercase">WPM</span></div>
                    <div className="flex flex-col md:block"><span className={`text-lg font-black ${r.accuracy >= 90 ? "text-emerald-400" : "text-blue-400"}`}>{r.accuracy}%</span><span className="text-[10px] font-bold text-foreground/30 ml-1 uppercase">Acc.</span></div>
                    <div className="col-span-2 md:col-span-1 flex justify-end"><span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border transition-all ${rt.color} ${rt.bg} ${rt.border} group-hover:scale-105`}>{rt.label}</span></div>
                  </div>
                );
              })}
            </div>
            ...
            {totalPages > 1 && (
              <div className="mt-4 flex items-center justify-between px-4 py-3 bg-card/20 border border-border rounded-2xl backdrop-blur-sm">
                <span className="text-[10px] font-mono font-bold text-foreground/20 uppercase tracking-widest">Page {currentPage} of {totalPages}</span>
                <div className="flex items-center gap-2">
                  <button 
                    disabled={currentPage === 1}
                    onClick={() => setCurrentPage(p => p - 1)}
                    className="px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-widest border border-border text-foreground/40 hover:text-foreground hover:bg-foreground/5 disabled:opacity-20 transition-all"
                  >
                    Prev
                  </button>
                  <button 
                    ...
                    onClick={() => setCurrentPage(p => p + 1)}
                    className="px-4 py-2 rounded-xl text-[11px] font-bold uppercase tracking-widest bg-orange-400 text-white shadow-lg shadow-orange-400/20 disabled:opacity-20 transition-all"
                  >
                    Next
                  </button>
                </div>
